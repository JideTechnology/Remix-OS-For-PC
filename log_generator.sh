#! /system/bin/sh
#SET VARS FOR LOGFILE NAMING in format: <LOGTYPE>_<DEVICE MODEL>_<REMIX VERSION>_<REMIX ARCH>_<DATE>.TXT
...
# WELCOME MESSAGE
print "\nRemix Log Dumper v0.5\n\nLogs will be dumped into 2 locations:\n\n1. $remix_sdcard_path - Remix File Manager has /sdcard named as My Remix.\n2. /RemixOS-Logs - directory created on the storage medium RemixOS is installed on.\n\nDumping log files to $remix_sdcard_path, please wait...\n"
# ACTUAL DUMPING
# SAVING TO INTERNAL REMIX STORAGE - /sdcard/RemixOS-Logs
...
if  [[ $native_partition ]]; then
	...
	if [[ $native_partition_fstype = "ntfs" ]]; then
		...
		if [[ $(ls $ntfs_mountpoint 2>/dev/null) ]]; then
			ntfs_path="$ntfs_mountpoint/RemixOS-Logs"
			mkdir -p $ntfs_path
